...
They have a list of the dimensions (length l, width w, and height h) of each present, and only want to order exactly as much as 
they need.
...
each gift a little easier: find the surface area of the box, which is 2*l*w + 2*w*h + 2*h*l. The elves also need a little extra paper 
...
line_list = (line.split('x') for line in lines)
input_elements = list(line_list)

arr = [list(map(int, item)) for item in input_elements]
...
for item in arr:
    ...
    items = [l, w, h]
    items.sort()
    perimeter = items[0] + items[0] + items[1] + items[1]
    surface_area = (2 * lw) + (2 * wh) + (2 * hl)
    total_square_feet += int(surface_area + min(lw, wh, hl))
    total_feet_ribbon += int(perimeter + (l * w * h))
